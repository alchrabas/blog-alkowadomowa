...
def plot_m_nalew_by_t():
    df = pd.read_csv('sliwkowka_m_nalew_by_t.csv')
    plt.plot(df.t, df.m, '.-')
    plt.xlabel('czas t [h]')
    plt.ylabel('masa nalewu [g]')

    plt.savefig("sliwkowka_m_nalew_by_t.png")
    plt.show()
